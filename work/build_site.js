...
function loadOverrides(catalogTs){try{const saved=JSON.parse(localStorage.getItem('lensColorOverrides')||'{}');const savedTs=localStorage.getItem('lensColorOverrides_catalogTs');if(savedTs&&savedTs!==catalogTs){console.warn('Catalog updated, clearing stale local overrides');localStorage.removeItem('lensColorOverrides');localStorage.removeItem('lensColorOverrides_catalogTs');return{}}localStorage.setItem('lensColorOverrides_catalogTs',catalogTs);return saved}catch(err){console.warn('Could not read manual overrides',err);return{}}}
...
async function init(){const [catalog,insights]=await Promise.all([fetch('./data/catalog.json?v=hex-calibrated').then(r=>r.json()),fetch('./data/insights.json?v=hex-calibrated').then(r=>r.json())]);state.catalog=catalog;state.insights=insights;state.overrides=loadOverrides(catalog.generatedAt);applyOverrides();renderTop();bindNav();renderAll()}
...
function normalizedStackBar(title,rows,segments,colors,kind){return '<section class="panel"><h2>'+title+'</h2><p class="chartNote">各品牌条形等宽（100%），显示类别比例。数字为 SKU 数。</p><div class="barList">'+rows.map(row=>{const total=sumValues(row.counts)||1;return '<div class="barRow"><button class="legendBtn" data-'+kind+'="'+safe(row.key)+'"><b>'+safe(row.label)+'</b></button><div><div class="stackBar" style="max-width:100%">'+segments.map(s=>'<button class="stackSeg" title="'+safe((CAT_LABEL[s]||HUE_LABEL[s]||s)+': '+(row.counts[s]||0)+' ('+((row.counts[s]||0)/total*100).toFixed(0)+'%)')+'" data-'+kind+'="'+safe(row.key)+'" data-seg="'+safe(s)+'" style="width:'+((row.counts[s]||0)/total*100).toFixed(1)+'%;background:'+(colors[s]||'#999')+'"></button>').join('')+'</div><div class="small">'+segments.map(s=>safe(CAT_LABEL[s]||HUE_LABEL[s]||s)+' '+((row.counts[s]||0)/total*100).toFixed(0)+'%').join(' · ')+'</div></div><b>'+total+'</b></div>'}).join('')+'</div></section>'}
function brandHueChart(title,brandHueRows,hueOrder,hueColors){return '<section class="panel"><h2>'+title+'</h2><p class="chartNote">各品牌条形等宽，色块从多到少排列；鼠标停留查看占比。</p><div class="barList">'+brandHueRows.map(row=>{const total=sumValues(row.counts)||1;const segs=Object.entries(row.counts).filter(([h,n])=>n>0).sort((a,b)=>b[1]-a[1]).map(([h])=>h);const top3=segs.slice(0,3).map(h=>safe(HUE_LABEL[h]||h)+' '+((row.counts[h]||0)/total*100).toFixed(0)+'%').join(' · ');return '<div class="barRow"><b>'+safe(row.label)+'</b><div><div class="stackBar" style="max-width:100%">'+segs.map(h=>'<button class="stackSeg" title="'+safe(HUE_LABEL[h]||h)+': '+(row.counts[h]||0)+' ('+((row.counts[h]||0)/total*100).toFixed(0)+'%)" data-hue="'+safe(h)+'" style="width:'+((row.counts[h]||0)/total*100).toFixed(1)+'%;background:'+(hueColors[h]||'#999')+'"></button>').join('')+'</div><div class="small" style="white-space:normal;line-height:1.6;margin-top:3px">'+top3+'</div></div><b>'+total+'</b></div>'}).join('')+'</div></section>'}
function heatMatrix(rows,cols,getVal,max){return '<div class="heatWrap"><div class="heat"><div class="heatRow"><div></div>'+cols.map(c=>'<div class="heatHead">'+safe(c)+'</div>').join('')+'</div>'+rows.map(r=>'<div class="heatRow"><div class="heatHead">'+safe(r)+'</div>'+cols.map(c=>{const v=getVal(r,c);const a=v/max;return '<div class="heatCell" style="background:rgba(30,28,25,'+(0.08+a*.62).toFixed(2)+')">'+(v||'')+'</div>'}).join('')+'</div>').join('')+'</div></div>'}
...
function spectrumChart(rows){const BRAND_C={'Warby Parker':'#c8763a','MOSCOT':'#3d6b4f','Gentle Monster':'#7b3f8c','Meta AI Glasses':'#3a72a8'};const brands=Object.keys(BRAND_C);const parsed=rows.map(r=>({row:r,hsl:hexToHsl(r.hueHex)})).filter(x=>x.hsl);const bins=Array.from({length:24},(_,i)=>({start:i*15,byBrand:Object.fromEntries(brands.map(b=>[b,0]))}));const neutrals={black:0,grey:0,light:0};const brandHexCounts=Object.fromEntries(brands.map(b=>[b,{}]));for(const item of parsed){const {h,s,l}=item.hsl;const br=item.row.brand;const hex=item.row.hueHex;if(brandHexCounts[br])brandHexCounts[br][hex]=(brandHexCounts[br][hex]||0)+1;if(l<18)neutrals.black++;else if(s<12)neutrals.grey++;else if(l>82&&s<28)neutrals.light++;else{const idx=Math.min(23,Math.floor(h/15));if(bins[idx].byBrand[br]!==undefined)bins[idx].byBrand[br]++}}const maxBin=Math.max(...bins.map(b=>brands.reduce((s,br)=>s+b.byBrand[br],0)),1);const barsHtml=bins.map(b=>{const total=brands.reduce((s,br)=>s+b.byBrand[br],0);const h=b.start+7.5;const ht=(8+total/maxBin*170).toFixed(1);const segs=brands.filter(br=>b.byBrand[br]>0).map(br=>'<span style="display:block;height:'+((b.byBrand[br]/total)*100).toFixed(1)+'%;background:'+BRAND_C[br]+';opacity:.9"></span>').join('');return '<div class="spectrumBin" style="height:'+ht+'px;background:hsl('+h+' 55% 55%);padding:0;overflow:hidden;display:flex;flex-direction:column-reverse;position:relative" title="'+b.start+'–'+(b.start+15)+'° total:'+total+'&#10;'+brands.filter(br=>b.byBrand[br]).map(br=>br+': '+b.byBrand[br]).join('&#10;')+'">'+segs+'<span style="position:absolute;left:50%;top:-16px;transform:translateX(-50%);font-size:9px;color:var(--mut);white-space:nowrap">'+(total||'')+'</span></div>'}).join('');const brandHexHtml=brands.map(b=>{const topHex=Object.entries(brandHexCounts[b]).sort((a,c)=>c[1]-a[1]).slice(0,4);return '<div style="margin-bottom:12px"><b style="color:'+BRAND_C[b]+'">'+b+'</b><div class="hexSwatches" style="margin-top:5px">'+topHex.map(([hex,n])=>'<div class="hexSwatch"><span class="hexBox" style="background:'+hex+'"></span><span>'+hex+'</span><b>'+n+'</b></div>').join('')+'</div></div>'}).join('');const legend=brands.map(b=>'<span class="markChip"><span class="markDot" style="background:'+BRAND_C[b]+'"></span>'+b+'</span>').join('');return '<section class="panel" style="margin-top:16px"><h2>镜片色 Hex Spectrum 分布 <span class="mut">按品牌分层</span></h2><p class="chartNote"><b>hueHex 均为镜片颜色编码，非镜架颜色。</b> Warby Parker 使用 15 种标准化镜片参考色；Gentle Monster 由产品图片镜片区域取样；MOSCOT 标准款镜片默认 G-15 绿色（#3f5942），CUSTOM MADE TINTS 使用实际镜片色；Meta 使用官方镜片命名对应色。每根柱子按品牌分层着色。</p><div class="spectrumLayout"><div><div class="colorMarks" style="margin-bottom:8px">'+legend+'</div><div class="spectrumBars" style="align-items:flex-end">'+barsHtml+'</div><div class="spectrumAxis"><span>0° Red</span><span>60° Yellow</span><span>120° Green</span><span>180° Cyan</span><span>240° Blue</span><span>300° Violet</span></div><div class="colorMarks" style="margin-top:10px"><span class="markChip"><span class="markDot" style="background:#171717"></span>Black neutral <b>'+neutrals.black+'</b></span><span class="markChip"><span class="markDot" style="background:#777"></span>Grey neutral <b>'+neutrals.grey+'</b></span><span class="markChip"><span class="markDot" style="background:#d8e2e2"></span>Light neutral <b>'+neutrals.light+'</b></span></div></div><div><h3>各品牌 Top Lens Hex</h3>'+brandHexHtml+'</div></div></section>'}
function renderOverview(){const root=$('#overview');const recs=state.catalog.records;const knownRecs=recs.filter(r=>r.lensHue!=='unknown');const hueRows=state.insights.hueSummary.filter(r=>r.hue!=='unknown').sort((a,b)=>b.brandCount-a.brandCount||b.count-a.count);const catCounts=group(recs,'cat');const frameCounts=group(recs,'frameFamily');const topHue=hueRows[0];const gradientRows=knownRecs.filter(r=>r.cat==='gradient');const gradientHue=sortedEntries(group(gradientRows,'lensHue'));const catColors={sun:'#2f2f2f',tint:'#bd6f92',gradient:'#44799a'};const maxHue=Math.max(...hueRows.map(r=>r.count),1);const maxGrad=Math.max(...gradientHue.map(x=>x[1]),1);const brandCounts=group(recs,'brand');const brandCatRows=Object.keys(brandCounts).map(b=>({key:b,label:b,counts:group(recs.filter(r=>r.brand===b),'cat')}));const brandHueRows=Object.keys(brandCounts).map(b=>({key:b,label:b,counts:group(knownRecs.filter(r=>r.brand===b),'lensHue')}));const frameCols=sortedEntries(frameCounts).slice(0,9).map(x=>x[0]);const heatRows=hueRows.slice(0,9).map(x=>x.hue);const pairMap={};knownRecs.forEach(r=>{const k=r.lensHue+'|'+r.frameFamily;pairMap[k]=(pairMap[k]||0)+1});const maxPair=Math.max(...Object.values(pairMap),1);const brandLensRows=Object.keys(brandCounts).map(b=>{const bRecs=recs.filter(r=>r.brand===b);const distinctHues=[...new Set(bRecs.map(r=>r.lensHue).filter(h=>h&&h!=='unknown'))].sort();const distinctLens=[...new Set(bRecs.map(r=>r.lensName))].sort();return '<tr><td><b>'+safe(b)+'</b></td><td style="text-align:center">'+bRecs.length+'</td><td style="text-align:center"><b>'+distinctHues.length+'</b></td><td>'+distinctHues.map(h=>'<span style="display:inline-flex;align-items:center;gap:3px;margin:2px 6px 2px 0;white-space:nowrap"><span style="width:9px;height:9px;border-radius:50%;background:'+(HUE_COLOR[h]||'#999')+';flex:0 0 auto"></span>'+safe(HUE_LABEL[h]||h)+'</span>').join('')+'</td><td style="color:#a09589;font-size:11px;max-width:220px">'+safe(distinctLens.slice(0,8).join('、'))+(distinctLens.length>8?'…':'')+'</td></tr>'}).join('');const catCompact='<section class="panel"><h2>镜片类别标准</h2><table class="table"><thead><tr><th>类别</th><th>透光率</th><th>分类</th></tr></thead><tbody><tr><td><b>Cat. 1–2</b></td><td><b>19–80%</b><br><span class="mut" style="font-size:10px">浅色 / 时尚镜片</span></td><td><span class="badge" style="background:#bd6f92">Tint</span></td></tr><tr><td><b>Cat. 3</b></td><td><b>8–18%</b><br><span class="mut" style="font-size:10px">强遮光 / 户外必备</span></td><td><span class="badge" style="background:#2f2f2f">Sun</span></td></tr><tr><td><b>Gradient</b></td><td><b>顶部 8–18%<br>底部 19–80%</b><br><span class="mut" style="font-size:10px">上深下浅渐变</span></td><td><span class="badge" style="background:#44799a">Gradient</span></td></tr></tbody></table></section>';root.innerHTML='<div class="metricGrid">'+metric('Total SKUs',recs.length)+metric('可分类色相',knownRecs.length)+metric('最高频色相',topHue?HUE_LABEL[topHue.hue]+' · '+topHue.count+'条':'—')+metric('Top 镜架家族',sortedEntries(frameCounts)[0][0])+'</div><div class="grid2" style="margin-top:14px"><section class="panel"><h2>各品牌镜片色相 <span class="mut">各品牌实际 offer 的色相种类</span></h2><p class="chartNote">色相由 lensHue 统计，排除 unknown。产品图片展示镜架+镜片整体，视觉主色由镜架主导。</p><table class="table"><thead><tr><th>品牌</th><th style="text-align:center">SKU</th><th style="text-align:center">色相数</th><th>色相分布</th><th style="color:#a09589">原始镜片命名</th></tr></thead><tbody>'+brandLensRows+'</tbody></table><div style="margin-top:10px;font-size:11px;color:#8a7f74;border-top:1px solid var(--line);padding-top:8px"><b>数据说明：</b>Warby Parker — 官方标准化镜片体系，15 种镜片类型跨所有镜架共用。MOSCOT — 标准 Sun 款原始命名为镜架醋酸颜色（如 Flesh、Crystal、Tortoise），实际镜片默认为 G-15 绿色；CUSTOM MADE TINTS / MONOCHROME 系列例外，有独立镜片色。Gentle Monster — 原始数据仅区分 Sun lens / Tint lens，色相由图片取样 hex 推导。Meta — 官方命名包含镜片类型（如 Transitions、Sapphire、Ruby）。</div></section><div style="display:flex;flex-direction:column;gap:12px">'+catCompact+(()=>{const tot=sumValues(catCounts)||1;return'<section class="panel"><h2>类别构成</h2><div class="barList">'+Object.entries(catCounts).sort((a,b)=>b[1]-a[1]).map(([cat,n])=>{const pct=(n/tot*100).toFixed(1);return'<div class="barRow"><b><span class="badge" style="background:'+catColors[cat]+'">'+safe(CAT_LABEL[cat]||cat)+'</span></b><div class="barTrack"><div class="barFill" style="width:'+pct+'%;background:'+catColors[cat]+'"></div></div><b>'+pct+'%<span class="mut" style="font-size:11px;margin-left:5px">'+n+'</span></b></div>'}).join('')+'</div></section>'})()+'</div></div><div class="chartGrid" style="margin-top:14px">'+barChart('色相分布排名',hueRows.map(r=>[r.hue,r.count]),maxHue,'hue')+normalizedStackBar('各品牌类别构成',brandCatRows,['sun','tint','gradient'],catColors,'brand')+brandHueChart('各品牌色相偏好',brandHueRows,state.insights.hueOrder.filter(h=>h!=='unknown'),HUE_COLOR)+'</div><section class="panel" style="margin-top:14px"><h2>镜片色相 × 镜架家族 搭配热力图</h2><p class="chartNote">颜色越深代表该搭配组合越常见。</p>'+heatMatrix(heatRows,frameCols,(h,f)=>pairMap[h+'|'+f]||0,maxPair)+'</section>'+spectrumChart(knownRecs)+(()=>{const catDefs=[{cat:'sun',color:'#2f2f2f',label:'Sun'},{cat:'tint',color:'#bd6f92',label:'Tint'},{cat:'gradient',color:'#44799a',label:'Gradient'}];const brandList=Object.keys(brandCounts);const cols=catDefs.map(({cat,color,label})=>{const catRecs=knownRecs.filter(r=>r.cat===cat);const catTotal=catRecs.length||1;const allHues=sortedEntries(group(catRecs,'lensHue'));const hueBarList='<div style="margin-bottom:14px;padding-bottom:12px;border-bottom:2px solid var(--line)"><div style="font-size:11px;font-weight:800;color:var(--mut);text-transform:uppercase;letter-spacing:.06em;margin-bottom:7px">整体色相分布</div><div style="display:grid;gap:5px">'+allHues.map(([h,n])=>{const pct=(n/catTotal*100).toFixed(1);return'<div style="display:grid;grid-template-columns:80px 1fr 46px;gap:6px;align-items:center"><span style="display:inline-flex;align-items:center;gap:5px;font-size:11px"><span style="width:10px;height:10px;border-radius:50%;background:'+(HUE_COLOR[h]||'#999')+';flex:0 0 auto;border:1px solid rgba(0,0,0,.12)"></span>'+safe(HUE_LABEL[h]||h)+'</span><div class="barTrack"><div class="barFill" style="width:'+pct+'%;background:'+(HUE_COLOR[h]||'#999')+'"></div></div><span style="font-size:11px;font-weight:800;text-align:right">'+pct+'%</span></div>'}).join('')+'</div></div>';const rows=brandList.map(brand=>{const bRecs=catRecs.filter(r=>r.brand===brand);if(!bRecs.length)return'';const hues=sortedEntries(group(bRecs,'lensHue'));const dots=hues.map(([h,n])=>'<span style="display:inline-flex;align-items:center;gap:3px;margin:0 8px 5px 0;white-space:nowrap"><span style="width:11px;height:11px;border-radius:50%;background:'+(HUE_COLOR[h]||'#999')+';flex:0 0 auto;border:1px solid rgba(0,0,0,.12)"></span><span style="font-size:11px">'+safe(HUE_LABEL[h]||h)+'</span><b style="font-size:11px">'+n+'</b></span>').join('');return'<div style="margin-bottom:11px;padding-bottom:11px;border-bottom:1px solid var(--line)"><div style="font-size:12px;font-weight:800;margin-bottom:5px">'+safe(brand)+'</div><div style="display:flex;flex-wrap:wrap">'+dots+'</div></div>'}).filter(Boolean).join('');return'<div class="panel"><h2 style="display:flex;align-items:center;gap:8px"><span class="badge" style="background:'+color+'">'+label+'</span><span style="font-size:13px;font-weight:normal;color:var(--mut)">'+catTotal+' SKU</span></h2>'+hueBarList+rows+'</div>'}).join('');return'<section style="margin-top:14px"><h2 style="margin:0 0 10px;font-size:18px">各类别色相选择 <span class="mut" style="font-size:13px;font-weight:normal">Sun / Tint / Gradient 中各品牌选了哪些颜色</span></h2><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px">'+cols+'</div></section>'})()+'<section class="panel" style="margin-top:14px"><h2>色相导航 <span class="mut">点击进入 Hue Lab</span></h2><div class="swatchBoard">'+hueRows.map(r=>'<button class="swatch" style="background:'+HUE_COLOR[r.hue]+'" data-hue="'+r.hue+'">'+HUE_LABEL[r.hue]+'<br><span style="font-size:10px;opacity:.8">'+r.count+'</span></button>').join('')+'</div></section>';root.querySelectorAll('[data-hue]').forEach(b=>b.onclick=()=>openHue(b.dataset.hue));root.querySelectorAll('[data-cat]').forEach(b=>b.onclick=()=>openCatalog({cat:b.dataset.cat}));root.querySelectorAll('[data-brand]').forEach(b=>b.onclick=()=>openCatalog({brand:b.dataset.brand}));root.querySelectorAll('.stackSeg[data-hue]').forEach(b=>b.onclick=(e)=>{e.stopPropagation();openHue(b.dataset.hue)})}
...
function renderHueLab(){const root=$('#hue');const d=state.insights.byHue[state.hue];const hueColor=HUE_COLOR[state.hue]||'#999';const totalRecs=Object.values(d.brandCounts).reduce((a,b)=>a+b,0);const brandRows=Object.entries(d.brandCounts).sort((a,b)=>b[1]-a[1]).map(([b,n])=>'<div class="barRow"><b>'+safe(b)+'</b><div class="barTrack"><div class="barFill" style="width:'+(n/totalRecs*100).toFixed(1)+'%;background:'+hueColor+'"></div></div><b>'+n+'</b></div>').join('');const catRows=Object.entries(d.categoryCounts).sort((a,b)=>b[1]-a[1]).map(([c,n])=>'<div class="barRow"><b>'+safe(CAT_LABEL[c]||c)+'</b><div class="barTrack"><div class="barFill" style="width:'+(n/totalRecs*100).toFixed(1)+'%;background:'+hueColor+'"></div></div><b>'+n+'</b></div>').join('');const SRC_PRIORITY={'gm-lens-image-hex':0,'source-hex':1,'reviewed-correction':2,'name-to-reference-hex':3,'name-keyword':4};const imageExamples=state.catalog.records.filter(r=>r.lensHue===state.hue&&r.img&&r.brand!=='Warby Parker').sort((a,b)=>(SRC_PRIORITY[a.hueSource]??5)-(SRC_PRIORITY[b.hueSource]??5)||a.rank-b.rank).filter((r,i,arr)=>arr.findIndex(x=>x.brand===r.brand&&x.product===r.product&&x.lensName===r.lensName)===i).slice(0,12);const imgNote=imageExamples.length?'<p class="chartNote">图片来源：GM 由产品图镜片区域取样 · MOSCOT/Meta 使用官方镜片色。Warby Parker 因产品图以镜架为主、镜片色不易辨认，不在此展示。</p>':'<p class="chartNote">此色相暂无适合展示的非 Warby Parker 图片样本。</p>';root.innerHTML=hueButtons()+'<div style="display:flex;align-items:center;gap:16px;background:'+hueColor+'18;border:1px solid '+hueColor+'55;border-radius:8px;padding:14px 18px;margin-bottom:14px"><div style="width:52px;height:52px;border-radius:10px;background:'+hueColor+';flex:0 0 auto;border:1px solid rgba(0,0,0,.15)"></div><div><b style="font-size:16px">'+safe(HUE_LABEL[state.hue])+'</b><p style="margin:3px 0 0;font-size:12px;color:#6a6058">'+totalRecs+' 条记录 · '+d.variants.length+' 种镜片命名 · '+Object.keys(d.brandCounts).length+' 个品牌</p></div></div><div class="detailLayout"><aside class="panel"><h2>镜片命名列表</h2><div class="variantList">'+d.variants.slice(0,80).map(v=>'<div class="variant"><div><b>'+safe(v.name)+'</b><span class="small">'+v.brands.join(' / ')+'</span></div><b>'+v.count+'</b></div>').join('')+'</div></aside><div style="display:flex;flex-direction:column;gap:12px"><section class="panel"><h2>品牌分布</h2><div class="barList">'+brandRows+'</div></section><section class="panel"><h2>类别构成</h2><div class="barList">'+catRows+'</div></section></div></div>'+(imageExamples.length?'<section class="panel" style="margin-top:14px"><h2>镜片色案例图</h2>'+imgNote+'<div class="imageGrid">'+imageExamples.map(productCard).join('')+'</div></section>':'<section class="panel" style="margin-top:14px"><h2>镜片色案例图</h2>'+imgNote+'</section>');root.querySelectorAll('.hueBtn').forEach(b=>b.onclick=()=>{state.hue=b.dataset.hue;renderHueLab()});bindManualSave(root)}
