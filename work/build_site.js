...
function brandHueChart(title,brandHueRows,hueOrder,hueColors){return '<section class="panel"><h2>'+title+'</h2><p class="chartNote">各品牌条形等宽，显示色相构成比例。点击色块进入 Hue Lab。</p><div class="barList">'+brandHueRows.map(row=>{const total=sumValues(row.counts)||1;const segs=hueOrder.filter(h=>row.counts[h]);return '<div class="barRow"><b>'+safe(row.label)+'</b><div><div class="stackBar" style="max-width:100%">'+segs.map(h=>'<button class="stackSeg" title="'+safe(HUE_LABEL[h]||h)+': '+(row.counts[h]||0)+' ('+((row.counts[h]||0)/total*100).toFixed(0)+'%)" data-hue="'+safe(h)+'" style="width:'+((row.counts[h]||0)/total*100).toFixed(1)+'%;background:'+(hueColors[h]||'#999')+'"></button>').join('')+'</div><div class="small">'+segs.slice(0,4).map(h=>safe(HUE_LABEL[h]||h)+' '+((row.counts[h]||0)/total*100).toFixed(0)+'%').join(' · ')+(segs.length>4?' …':'')+'</div></div><b>'+total+'</b></div>'}).join('')+'</div></section>'}
