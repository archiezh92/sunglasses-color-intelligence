...
function applyOverrides(){const map=state.overrides||{};(state.catalog?.records||[]).forEach(r=>{if(r.lensHue==='olive')r.lensHue='green';const o=map[r.id];if(o){if(o.lensHue)r.lensHue=normalizeHue(o.lensHue);if(o.cat)r.cat=o.cat;r.hueSource='manual-override'}})}
...
function saveManualOverride(id,hue,cat){const r=state.catalog.records.find(x=>x.id===id);if(!r){alert('Could not find this product in the catalog.');return}state.overrides={...(state.overrides||{}),[id]:{id,brand:r.brand,product:r.product,lensName:r.lensName,frameColor:r.frameColor,lensHue:normalizeHue(hue),cat,hueHex:r.hueHex||'',hueSource:'manual-override',updatedAt:new Date().toISOString()}};persistOverrides();applyOverrides();renderCatalog();renderTop();showSaved('Saved in this page. Export once when done.')}
...
function productCard(r){const sw=r.hueHex?'<span class="calibSw" style="background:'+r.hueHex+'"></span>':'';const hueOptions=Object.keys(HUE_LABEL).map(h=>'<option value="'+h+'" '+(r.lensHue===h?'selected':'')+'>'+HUE_LABEL[h]+'</option>').join('');const catOptions=Object.keys(CAT_LABEL).map(c=>'<option value="'+c+'" '+(r.cat===c?'selected':'')+'>'+CAT_LABEL[c]+'</option>').join('');const manual=r.hueSource==='manual-override'?'<span class="overrideNote">manual</span>':'';return '<figure class="productCard" data-id="'+safe(r.id)+'"><img loading="lazy" src="'+r.img+'" alt="'+safe(r.product)+'"><figcaption><b>'+safe(r.brand)+'</b><br>'+safe(r.product)+'<br><span class="small">'+safe(r.lensName)+' / '+safe(r.frameColor)+'</span><br><span class="badge">'+CAT_LABEL[r.cat]+'</span>'+manual+'<div class="calib">'+sw+'<span>'+safe(r.hueHex||'no hex')+' · '+safe(HUE_LABEL[r.lensHue]||r.lensHue)+' · '+safe(r.hueSource||'unknown')+'</span></div><div class="manualFix"><select class="miniSelect manualHue">'+hueOptions+'</select><select class="miniSelect manualCat">'+catOptions+'</select><button class="miniBtn saveFix" type="button">Save</button></div></figcaption></figure>'}
